 ...
 */
class LeftSemiAntiJoinToSubqueryEquivalencySuite extends QueryTest with SharedSQLContext {
  ...
  private def checkLeftSemiOrAntiPlan(plan: LogicalPlan): Unit = {
    plan match {
      case j@Join(_, _, LeftSemiOrAnti(_), _, _) =>
  ...
  test("TC 2.9: LeftSemi over left semi join with correlated columns on the left table") {
    ...
    val optPlan = plan1.queryExecution.optimizedPlan.collectFirst {
      case j @ Join(_, _, LeftSemiOrAnti(_), _, _) => j
    }
    optPlan match {
      case Some(j@Join(_, _: Union, LeftSemiOrAnti(_), _, _)) =>
      // This is the expected result.
      case _ =>
        fail(
          s"""
