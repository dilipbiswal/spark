...
class HiveCommandSuite extends QueryTest with SQLTestUtils with TestHiveSingleton {
   protected override def beforeAll(): Unit = {
    ...
    sql(
      """
        |CREATE EXTERNAL TABLE parquet_t0 (c1 INT, c2 STRING)
        |USING org.apache.spark.sql.parquet.DefaultSource
      """.stripMargin)

     sql(
      """
        |CREATE EXTERNAL TABLE parquet_t1 (c1 INT, c2 STRING)
        |STORED AS PARQUET
  ...
  test("show tblproperties of data source tables - basic") {
    checkAnswer(
      sql("SHOW TBLPROPERTIES parquet_t0")
    ...
    checkAnswer(
      sql("SHOW TBLPROPERTIES parquet_t0('spark.sql.sources.schema.numParts')"),
      Row("1"))
  }
  ...
  test("show tblproperties for datasource table - errors") {
    ...
    val message2 = intercept[AnalysisException] {
      sql("SHOW TBLPROPERTIES parquet_t0('invalid.property.key')")
    }.getMessage
    assert(
      message2.contains(s"Table default.parquet_t0 does not have property: invalid.property.key"))
  }

  test("show tblproperties for hive table") {
    checkAnswer(sql("SHOW TBLPROPERTIES parquet_t1('prop1Key')"), Row("prop1Val"))
    checkAnswer(sql("SHOW TBLPROPERTIES parquet_t1('`prop2Key`')"), Row("prop2Val"))
  }
